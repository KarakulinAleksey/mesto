export default class FormValidator {
  constructor(validationConfig, сheckForm){
    ...
    this._inputList = Array.from(this._checkForm.querySelectorAll(this._inputSelector)); //массив проверяемых инпутов
    this._buttonElement = this._checkForm.querySelector(this._submitButtonSelector); //кнопка сохранить
}

//функция изменения цвета input и вывода текста сообщения об ошибке
_showInputError(inputElement, errorMessage){
  const nameInputError = this._checkForm.querySelector(`.${inputElement.name}-input-error`);
